var Rollup = (function (exports) {
	...
	function escapeHTML(txt) {
	    ...
	    return String(txt).replace(/[&<>"'`=/]/g, function (char) { return escapeHtmlMapping[char] || char; });
	}
	...
	class CellManager {
	        ...
	        if ($cell) {
	            ...
	            if (editor) {
	                ...
	                promise = valuePromise.then(function (value) {
	                    ...
	                    this$1.updateCell(colIndex, rowIndex, value, true);
	                    $cell.focus();
	    ...
	    pasteContentInCell(data) {
	        ...
	        matrix.forEach(function (row, i) {
	            ...
	            row.forEach(function (cell, j) {
	                ...
	                this$1.updateCell(colIndex, rowIndex, cell, true);
	            });
	        });
	    }
	    ...
	    updateCell(colIndex, rowIndex, value, refreshHtml) {
	        if ( refreshHtml === void 0 ) refreshHtml = false;
	    ...
	    getCellContent(cell, refreshHtml) {
	        if ( refreshHtml === void 0 ) refreshHtml = false;
	        ...
	        var customFormatter = CellManager.getCustomCellFormatter(cell);
	        var contentHTML;
	...
	class RowManager {
	    ...
	    refreshRow(row, rowIndex) {
	        ...
	        _row.forEach(function (cell) {
	            this$1.cellmanager.refreshCell(cell, true);
	        });
	    }
	    ...
	    checkAll(toggle) {
	        ...
	        if (toggle) {
	            if (this.datamanager._filteredRows) {
	                this.datamanager._filteredRows.forEach(function (f) {
	                    this$1.checkRow(f, toggle);
	                });
	            } else {
	...
	function filterRows(rows, filters) {
	    ...
	    var loop = function ( colIndex ) {
	        ...
	        var filterMethod = getFilterMethod(rows, filter);
	...
	function getFilterMethod(rows, filter) {
	    var getFormattedValue = function (cell) {
	        var formatter = CellManager.getCustomCellFormatter(cell);
	    ...
	    var stringCompareValue = function (cell) { return String(stripHTML(cell.html || '') || getFormattedValue(cell)).toLowerCase(); };
	...
	function guessFilter(keyword) {
	    ...
	    if (keyword.split(':').length === 2 && keyword.split(':').every(function (v) { return isNumber(v.trim()); })) {
			...
			if (this.report_name) {
				return this.get_report_doc()
					.then(function (doc) {
						...
						this$1.page_title = __(this$1.report_name);
	...
	frappe.views.QueryReport = class QueryReport extends frappe.views.BaseList {
		...
		refresh(have_filters_changed) {
			...
			var are_default_filters = this.filters
				.map(function (filter) {
					return (
				...
				.every(function (res) { return res === true; });
			...
			return new Promise(function (resolve) {
				...
				if (data.custom_filters) {
					this$1.set_filters(data.custom_filters);
					this$1.previous_filters = data.custom_filters;
				}
		...
		get_filters_html_for_print() {
			...
			return Object.keys(applied_filters)
				.map(function (fieldname) {
					var docfield = frappe.query_report.get_filter(fieldname).df;
							...
							primary_action: function (values) {
								frappe.call({
									...
									args: {
										...
										filters: this$1.get_filter_values(),
									},
									callback: function(r) {
										this.show_save = false;
										d.hide();
	...
	frappe.ui.GroupBy = class {
		...
		get_group_by_fields() {
			...
			this.group_by_fields[this.doctype] = fields
				.concat(tag_field)
				.sort(function (a, b) { return __(a.label).localeCompare(__(b.label)); });
