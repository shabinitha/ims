	...
	erpnext.ProductView =  class {
		...
		*/
		...
		bind_filters() {
			...
			$('.filter-lookup-input').on('keydown', frappe.utils.debounce(function (e) {
				var $input = $(e.target);
				var keyword = ($input.val() || '').toLowerCase();
				var $filter_options = $input.next('.filter-options');

				$filter_options.find('.filter-lookup-wrapper').show();
				$filter_options.find('.filter-lookup-wrapper').each(function (i, el) {
					var $el = $(el);
					var value = $el.data('value').toLowerCase();
	...
	erpnext.ProductSearch = class {
		...
		bindSearchAction() {
			...
			$("body").on("click", function (e) {
				var searchEvent = $(e.target).closest(this$1.search_box_id).length;
