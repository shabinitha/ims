(function () {
	...
	frappe.ui.FilterGroup = class {
			...
			$(document.body).on('click', function (e) {
				if (this$1.wrapper && this$1.wrapper.is(':visible')) {
					...
					if (
						...
						this$1.wrapper && this$1.hide_popover();
					}
				}
			});
		...
		set_filter_events() {
			...
			this.wrapper.find(".apply-filters").on("click", function () { return this$1.hide_popover(); });
		}
	...
	frappe.ui.toggle_like = function($btn, doctype, name, callback) {
		...
		frappe.call({
			...
			callback: function(r) {
				...
				if (!r.exc) {
					for (var i = 0, list = document.querySelectorAll(".like-action"); i < list.length; i += 1) {
						var like = list[i];
	...
	frappe.views.ListView = class ListView extends frappe.views.BaseList {
		...
		get_like_html(doc) {
			...
			var heart_class = liked_by.includes(frappe.session.user)
				? "liked-by liked"
				: "not-liked";
			var title = liked_by.map(function (u) { return frappe.user_info(u).fullname; }).join(", ");

			var div = document.createElement("div");
			div.innerHTML = "\n\t\t\t<span class=\"like-action " + heart_class + "\">\n\t\t\t\t" + (frappe.utils.icon("heart", "sm", "like-icon")) + "\n\t\t\t</span>\n\t\t\t<span class=\"likes-count\">\n\t\t\t\t" + (liked_by.length > 99 ? __("99") + "+" : __(liked_by.length || "")) + "\n\t\t\t</span>\n\t\t";

			var like = div.querySelector(".like-action");
		...
		get_subject_html(doc) {
			...
			var checkbox = div.querySelector(".list-row-checkbox");
			checkbox.dataset.doctype = this.doctype;
			checkbox.dataset.name = doc.name;

			var link = div.querySelector(".level-item a");
	...
	frappe.views.ListSidebar = class ListSidebar {
		...
		make() {
			...
			if (this.list_view.list_view_settings && this.list_view.list_view_settings.disable_sidebar_stats) {
				...
				this.sidebar.find(".list-stats").on("show.bs.dropdown", function (e) {
					this$1.reload_stats();
				});
			}
		...
		get_stats() {
			...
			var dropdown_options = me.sidebar.find(".list-stats-dropdown .stat-result");
